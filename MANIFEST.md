...
SCA-1 does not replace the Core. It observes Core activity through EventJournal and maintains an independent continuity contour.

Current flow:

**Core → EventJournal → SCA-1 Shadow**

The Shadow maintains continuity identity, continuity state, versions, event sequence, hashes, an append-only ledger, rollback information and validity information.
...
The current integration reports `mode = shadow`, `version = SCA-1-SHADOW/1`, valid continuity ledger and `core_mutation = false`.

SCA-1 observes and records continuity; it does not replace the application Core.
...
The Core `PermissionGate` remains the application-facing permission interface.
...
Security integration remains additive and does not require moving all security logic into the Core.

## Permission Boundary

The Core permission system controls capabilities including screen access, microphone, camera, network sending, file writing, external AI, task execution and ambient listening.

Permissions remain explicit and are not automatically granted by network discovery or module presence.
...
**Quest → P2P → Execution → SecureSettlement → Economy/Escrow → EventJournal → SCA-1**

The system remains experimental and actively evolving.

Current development focuses on integration testing, regression testing, bug fixing, component boundaries, interaction verification and Core stability.

New architectural layers should not be introduced unless the existing system requires them.
...
The Core is the continuity of the Symbiont.

The Body provides capabilities.

The Bridge connects the Core with those capabilities.

External services and models may extend the system, but they should not define the identity of the Symbiont.

The architecture should evolve by extending existing boundaries rather than constantly rebuilding the Core.
