...
class NodeHandshake:
    ...
    def accept(self, packet):
        ...
        try:
            if not self.zerotrust.verify(packet):
                self.state = "REJECTED"
                return False
            ...
            if packet.get("payload", {}).get("action") != "HELLO":
                self.state = "REJECTED"
                return False
...
def get_node_handshake(node_identity, zerotrust=None, scp=None):
    return NodeHandshake(node_identity, zerotrust, scp)
